 ...
 * WrikeTokenExchange now writes this on every mint (including the re-issue that
 * happens when an existing row is updated), and the API Tokens table reads it.
 *
...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    ...
    // Best effort for rows that predate the column. A row's JWE was minted when
    // the row was created, or re-minted when the same user signed in again for
    // the same account and environment (which updates the row). created_at is
    // therefore the closest thing to an issue date that exists for them, and
    // 180 days is TOKEN_TTL_DAYS from src/utils/tokenTtl.js — spelled out here
    // because a migration has to mean the same thing when it is re-read later,
    // whatever that constant says by then.
